...
class CalciteToPekkopec extends STSpec {
  ...
  "calcite to pekko" should {
    "demo1" in {
      ...
      val future                       = source.runWith(Sink.foreach(row => println(s"Stream 输出：$row")))
    ...
    object RelNodeToPekkoStream {
      ...
      def scanTable(scan: TableScan): Iterator[Row]                         = Iterator.empty
      ...
      def convert(relNode: RelNode): Source[Row, NotUsed] = {
        relNode match {
          case scan: TableScan =>
            // 处理 TableScan
            Source.fromIterator(() => scanTable(scan))
          case filter: Filter =>
            // 处理 Filter
